...
class WTCGL {
  ...
  constructor(
    ...
    if (webgl2 === true) {
      this._ctx =
        this._el.getContext("webgl2", this.webgl_params) ||
        this._el.getContext("webgl", this.webgl_params) ||
        this._el.getContext("experimental-webgl", this.webgl_params);
    } else {
      this._ctx =
        this._el.getContext("webgl", this.webgl_params) ||
        this._el.getContext("experimental-webgl", this.webgl_params);
    ...
    // Create the shaders
    this._vertexShader = WTCGL.createShaderOfType(
      this._ctx,
      this._ctx.VERTEX_SHADER,
      vertexShaderSource
    );
    this._fragmentShader = WTCGL.createShaderOfType(
      this._ctx,
    ...
    if (!this._ctx.getProgramParameter(this._program, this._ctx.LINK_STATUS)) {
      console.log(
        "Unable to initialize the shader program: " +
          this._ctx.getProgramInfoLog(this._program)
      );
      return null;
    }
  ...
  addFrameBuffer(w, h, tiling = 0, buffertype = 0) {
    ...
    gl.bindTexture(gl.TEXTURE_2D, targetTexture);
    {
      ...
      const type =
        buffertype === WTCGL.TEXTYPE_FLOAT ? gl.FLOAT : gl.UNSIGNED_BYTE;
      const data = null;
      gl.texImage2D(
        gl.TEXTURE_2D,
        level,
  ...
  addTexture(name, type, image, liveUpdate = false) {
    ...
    if (type === WTCGL.IMAGETYPE_MIRROR) {
      this._ctx.texParameteri(
        this._ctx.TEXTURE_2D,
        this._ctx.TEXTURE_WRAP_S,
        this._ctx.MIRRORED_REPEAT
      );
      this._ctx.texParameteri(
        this._ctx.TEXTURE_2D,
        this._ctx.TEXTURE_WRAP_T,
        this._ctx.MIRRORED_REPEAT
      );
  ...
  render(buffer = {}) {
    ...
    this._ctx.viewport(
      0,
      0,
      buffer.w || this._ctx.viewportWidth,
      buffer.h || this._ctx.viewportHeight
    );
    if (this.clearing) {
    ...
    this.includePerspectiveMatrix &&
      this._ctx.uniformMatrix4fv(
        this._programInfo.uniforms.projectionMatrix,
        false,
        this.perspectiveMatrix
      );
    this.includeModelViewMatrix &&
      this._ctx.uniformMatrix4fv(
        this._programInfo.uniforms.modelViewMatrix,
        false,
   ...
   */
  get webgl_params() {
    return { alpha: true, antialias: true, preserveDrawingBuffer: true };
  }
