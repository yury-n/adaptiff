window.sketch = function(p) {
  ...
  p.setup = function() {
    ...
    p.stroke(window.strokeColor);
    p.strokeWeight(1);
  };
  p.draw = function() {
    if (window.withBackground) {
      p.background(window.backgroundColor);
    }
      ...
      for (let j = 0; j < flow_width; j++) {
        row.push(
          calculate_flow(
            (j +
              vertical_shift *
                p.floor((vertical_partitions * j) / flow_height)) *
              noise_size,
            (i +
              horizontal_shift *
                p.floor((horizontal_partitions * i) / flow_width)) *
              noise_size,
            noise_radius
          )
        );
  ...
  function calculate_flow(x, y, r) {
    ...
    for (var i = 0; i < radial_samples; i++) {
      ...
      let pos2 = p.createVector(
        x + p.cos(angle + p.PI) * r,
        y + p.sin(angle + p.PI) * r
      );
